            ...
            <table role="table" aria-busy="false" aria-colcount="3" class="table b-table table-striped table-hover" id="__BVID__6249"><!----><!---->
              ...
              <tbody role="rowgroup"><!---->
                <tr role="row" v-for="item in searhResult.items" :key="item.id">
                    ...
                    <td aria-colindex="2" role="cell" class=""><a :href="'/novel/'+item.book_id">{{ item.book_author }}</a></td>
                    <td aria-colindex="3" role="cell" class=""><a :href="'/novel/'+item.book_id+'/'+item.book_newest_url">{{ item.book_newest_name }}</a></td>
                </tr>
